...
submatch groups was doing, I looked closely at this group with an
**assertion**:
...
Notice the *say* statement is inside a block defined by curly braces.


### Dynamic variables

Grammar and actions make heavy use of dynamic variables (e.g., **$\*IN-DEFN-BLOCK**).  They show
their versatility when a variable needs to be changed deep in a parse
tree and that value is expected to hold during the remainder of that
parse (the *caller*) and subparse actions.
...
questions on **IRC #perl6-dev**, they are much clearer.

Basically, inside an action method, using *make* will assign the
current value to the *.ast* attribute of the match object with the
method's name. So, given the following method:
...
parent object's (usually a POD class) values have been evaluated or
...
test, and commit small changes! I got so wrapped up in the sometimes
crooked parse path of POD features that I made too many changes and
couldn't easily undo them.  I hope I don't repeat that mistake.

Have a Merry Christmas and Happy Perl 6 New Year!
